...
while True:
    ...
    if size == "1":
        dx=list(map(lambda x: float(x), input("tamanho de cada elemento em x: ").split(",")))
        dy=list(map(lambda y: float(y), input("tamanho de cada elemento em y: ").split(",")))
        break
    elif size == "0":
